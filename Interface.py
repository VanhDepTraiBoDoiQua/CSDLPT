...
def rangepartition(ratingstablename, numberofpartitions, openconnection):
    ...
    for i in range(numberofpartitions):
        ...
        partition_table_name = "range_part" + str(i)
...
def rangeinsert(ratingstablename, userid, itemid, rating, openconnection):
    ...
    no_of_partitions = get_partitions_count("range_part", openconnection)
    ...
    for i in range(no_of_partitions):
        ...
        if i == 0:
            if rating >= min_rating and rating <= max_rating:
                partition_table = "range_part" + str(i)
                cur.execute("INSERT INTO " + ratingstablename + " (userid, movieid, rating) VALUES (%s, %s, %s)",
                            (userid, itemid, rating))
                cur.execute("INSERT INTO " + partition_table + " (userid, movieid, rating) VALUES (%s, %s, %s)",
                            (userid, itemid, rating))
                break
        else:
            if rating > min_rating and rating <= max_rating:
